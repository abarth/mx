...
class socket : public handle<socket> {
 public:
  ...
  static mx_status_t create(socket* endpoint0,
                            socket* endpoint1,
                            uint32_t flags);

  mx_ssize_t write(uint32_t flags, mx_size_t size, const void* buffer) {
    return mx_socket_write(get(), flags, size, buffer);
  }

  mx_ssize_t read(uint32_t flags, mx_size_t size, void* buffer) {
